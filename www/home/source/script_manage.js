...
function myfriend_post(id, result) {
	if(result) {
		var ids = explode('_', id);
		var uid = ids[1];
		$('friend_'+uid).innerHTML = '<p> is now you friend, now you can：<a href="space.php?uid='+uid+'#comment" target="_blank">Leave a Message/Cooment</a> ，or <a href="cp.php?ac=poke&op=send&uid='+uid+'" id="a_poke_'+uid+'" onclick="ajaxmenu(event, this.id, 1)">Poke</a></p>';
	}
}
//删除好友请求
...
function feed_more_show(feedid) {
	...
	$(opid).innerHTML = '&laquo; Close List';
	$(opid).onclick = function() {
...
function feed_more_close(feedid) {
	...
	$(opid).innerHTML = '&raquo; More Feed';
	$(opid).onclick = function() {
