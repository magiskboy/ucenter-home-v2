...
function qf_checkauth() {
	global $_QFG, $_QSC;
	// Token sync từ URL: uid + sig=md5(uid.UC_KEY) – gateway đã checklogin, iframe nhận uid qua URL
	if (!empty($_GET['uid']) && !empty($_GET['sig']) && !empty($_QSC['UC_KEY'])) {
		$uid = (int)$_GET['uid'];
		if ($uid > 0 && $_GET['sig'] === md5((string)$uid . (string)$_QSC['UC_KEY'])) {
			$_QFG['uid'] = $uid;
			return '';
		}
	}
	...
	if($auth = $_COOKIE[$_QSC['cookiepre'].'auth']) {
		...
		$_QFG['uid'] = intval($uid);
		if($uid > 0 && $password) {
			if($_QFG['db']->result("SELECT password FROM pap_tbpre_session WHERE uid='$uid'") == $password) {
				$_QFG['uid'] = (int)$uid;
			}
		}
	}
	//for uch width pw
	if(!$_QFG['uid']) {
		if (!defined('UC_API') && !empty($_QSC['UC_API'])) define('UC_API', $_QSC['UC_API']);
		if (!defined('UC_KEY') && !empty($_QSC['UC_KEY'])) define('UC_KEY', $_QSC['UC_KEY']);
		if (!defined('UC_CONNECT')) define('UC_CONNECT', 'mysql');
